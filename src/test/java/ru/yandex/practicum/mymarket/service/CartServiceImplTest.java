...
class CartServiceImplTest {
    ...
    void shouldReturnCartItems() {
        ...
        List<CartItemDto> expected = List.of(
            TestData.cartItemDto()
        );

        when(cartItemRepository.findAllByOrderByIdAsc())
            .thenReturn(cartItems);

        when(cartItemMapper.toDtoList(cartItems))
            .thenReturn(expected);
    ...
    void shouldCalculateTotal() {
        ...
        CartItem first = TestData.cartItem(
            BigDecimal.valueOf(100),
            2
        );

        CartItem second = TestData.cartItem(
